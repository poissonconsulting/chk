...
chk_file <- function(x) {
  ...
  if(length(x) == 1L)
    abort(p0("Can't find file '", x, "'."))
  abort(p0("Can't find the following files: ", cc(x, " or "), "."))
}
...
chk_dir <- function(x){
  ...
  x <- x[!dir.exists(x)]
  if(length(x) == 1L)
    abort(p0("Can't find directory '", x, "'."))
  abort(p0("Can't find the following directories: ", cc(x, " or "), "."))
}
