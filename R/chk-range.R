...
chk_range <- function(x, range = c(0, 1), x_name = NULL) {
  ...
  if (length(x) == 1L) {
    if (range[1] == range[2]) {
      abort_chk(x_name, " must be ", cc(range[1]), ", not ", cc(x), x = x)
    }
    abort_chk(
  ...
  if (range[1] == range[2]) {
    abort_chk(x_name, " must have values of ", cc(range[1]), x = x)
  }
  abort_chk(x_name, " must have values between ", cc(range, " and "), x = x)
}
