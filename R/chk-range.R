...
chk_range <- function (x, range = c(0, 1), x_name = NULL) {
  ...
  if(length(x) == 1L) {
    ...
    abort_chk(x_name, " must be between ", cc(range, " and "), 
        ", not ", cc(x), ".")
  }
  if(range[1] == range[2])
...
chk_lte <- function (x, value = 0, x_name = NULL) {
  ...
  if(is.null(x_name))  x_name <- deparse_backtick(substitute(x))
  if(length(x) == 1L)
    abort_chk(x_name, " must be less than or equal to ", cc(value), ", not ", cc(x), 
         ".")
...
chk_gte <- function (x, value = 0, x_name = NULL) {
  ...
  if(is.null(x_name))  x_name <- deparse_backtick(substitute(x))
  if(length(x) == 1L)
    abort_chk(x_name, " must be greater than or equal to ", cc(value), 
        ", not ", cc(x), ".")
