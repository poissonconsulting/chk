context("chk-in")

test_that("vld_in", {
  expect_true(vld_in(integer(0), 1))
  expect_false(vld_in(1:3, 1:2))
  expect_true(vld_in(numeric(0), 1L))
  
  expect_false(vld_in(-1, c(0, 1, NA)))
  expect_false(vld_in(1L, 0L))
  expect_false(vld_in(1, NA)) 

  expect_false(vld_in(c(NA, 1), 1)) 
  expect_true(vld_in(NA, NA))
  
  expect_false(vld_in(c(NA, 1), 1:2)) 
})

test_that("chk_in", {
  expect_null(chk_in(integer(0), 1))
  expect_invisible(chk_in(integer(0), 1))
  expect_error(chk_in(NA, c(0, Inf)), "^`NA` must match 0 or Inf, not NA[.]$")
  expect_error(chk_in(1:3, c(0, 1, NA)), "`1:3` must have values matching 0, 1 or NA[.]$")
  expect_error(chk_in(-1, c(0, 1, NA)), "^`-1` must match 0, 1 or NA, not -1[.]$")
  expect_error(chk_in(c(1L, 1L), 0L), c("^`c[(]1L, 1L[)]` must have values matching 0[.]$"))
  expect_error(chk_in(1, ""), "^`1` must match '', not 1[.]$")
  expect_error(chk_in(1, NA), 
               "^`1` must match NA, not 1[.]$")
  expect_error(chk_in(c(NA, 1), 1), 
               "^`c[(]NA, 1[)]` must have values matching 1[.]$")
  expect_error(chk_in(c(NA, 1), 1:2), 
               "^`c[(]NA, 1[)]` must have values matching 1 or 2[.]$")
  expect_error(chk_in(3L, values = c(1L, 1L, 7L)), 
               "^`3L` must match 1 or 7, not 3[.]$")
  expect_error(chk_in(3L, values = c(1L, 1L, 7L), x_name = "1"), 
               "^1 must match 1 or 7, not 3[.]$")
})

test_that("vld_has", {
  expect_true(vld_has(1, 1))
  expect_true(vld_has(1:2, 1))
  expect_true(vld_has(1:3, 1:2))
})

test_that("chk_has", {
  expect_null(chk_has(1, 1))
  expect_invisible(chk_has(1, 1))
  expect_error(chk_has(1, 1:2), "^`1` must include 2[.]$")
  expect_error(chk_has(1, 1:3), "^`1` must include 2 and 3[.]$")
  expect_error(chk_has(2, 1), "^`2` must include 1[.]$")
})
