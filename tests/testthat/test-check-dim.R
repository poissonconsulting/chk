test_that("check_dim length", {
  ...
  expect_chk_error(check_dim(integer(0)),
...
test_that("check_dim nrow", {
  ...
  expect_chk_error(check_dim(data.frame(x = integer(0)), dim = nrow),
                   "`nrow[(]data.frame[(]x = integer[(]0[)][)][)]` must be greater than 0, not 0[.]")
  expect_chk_error(check_dim(data.frame(x = integer(0)), dim = nrow, dim_name = "ncol"),
                   "`ncol[(]data.frame[(]x = integer[(]0[)][)][)]` must be greater than 0, not 0[.]")
    expect_chk_error(check_dim(data.frame(x = integer(0)), dim = nrow, x_name = "y", dim_name = "ncol"),
