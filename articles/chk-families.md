...
The `vld_` functions are used within the `chk_` functions. The `chk_`
...
R](https://adv-r.hadley.nz/) (Wickham, 2019).

For reasons of space, the `x_name = NULL` argument is not shown. For a
more simplified list of the `chk` functions, you can see the
[Reference](https://poissonconsulting.github.io/chk/reference/index.html)
section.
...
![Classification of the chk functions by family](chk_diagram_II.png)

### Missing Input Checker

Check if the function input is missing or not

`chk_missing` function uses
[`missing()`](https://rdrr.io/r/base/missing.html) to check if an
argument has been left out when the function is called.

| Function | Code |
|:---|:---|
| [`chk_missing()`](https://poissonconsulting.github.io/chk/reference/chk_missing.md) | [`missing()`](https://rdrr.io/r/base/missing.html) |
| [`chk_not_missing()`](https://poissonconsulting.github.io/chk/reference/chk_not_missing.md) | `!missing()` |

### `...` Checker

Check if the function input comes from `...` (`dot-dot-dot`) or not

The functions `chk_used(...)` and `chk_unused(...)` check if any
arguments have been provided through `...` (called `dot-dot-dot` or
ellipsis), which is commonly used in R to allow a variable number of
arguments.
...
These `chk` functions check the existence of a file, the validity of its
extension, and the existence of a directory.
...
Check if the function input is a scalar. In R, scalars are vectors of
length 1.
...
The following functions check if the functions inputs are vectors of
length 1 of a particular data type. Each data type has a special syntax
...
which considers `TRUE` or `FALSE` as possible values, or use
...
Also you can check the time zone with
...
Vectors are a family of data types that come in two forms: atomic
vectors and lists. When vectors consist of elements of the same data
type, they can be considered atomic, matrices, or arrays. The elements
in a list, however, can be of different types.
...
are different from [`is.vector()`](https://rdrr.io/r/base/vector.html),
...
[`typeof()`](https://rdrr.io/r/base/typeof.html) to confirm the data
type.
...
For numbers there are four functions. R differentiates between doubles
...
which will detect both. The third type of number is complex
...
Consider that to explicitly create an integer in R, you need to use the
suffix `L`.
...
or a double that can be converted to an integer without changing its
value.
...
is a special case of `chk_whole_number`, differing in that it ensures
...
that allows detecting if the argument that the user is providing
contains strings.
...
Check if the function input has names and are valid `chk_named` function
works with vectors, lists, data frames, and matrices that have named
columns or rows. Do not confuse with `check_names`.

`chk_valid_name` function specifically designed to check if the elements
of a character vector are valid R names. If you want to know what is
...
are used to compare two objects, but they differ in how strict the
comparison is.

`chk_equal` and `chk_equivalent`checks if x and y are numerically
...
Check if the function input are numbers in increasing order `chk_sorted`
function checks if `x` is sorted in non-decreasing order, ignoring any
NA values.
...
The `setequal` function in R is used to check if two vectors contain
exactly the same elements, regardless of the order or number of
repetitions.
...
First, the `%in%` function is used to check whether the elements of a
vector `x` are present in a specified set of values. This returns a
logical vector, which is then simplified by
[`all()`](https://rdrr.io/r/base/all.html). The
[`all()`](https://rdrr.io/r/base/all.html) function checks if all values
...
all elements in the `x` vector are present in `values`. Similarly, for
`vld_` and
[`chk_superset()`](https://poissonconsulting.github.io/chk/reference/chk_superset.md),
it indicates that all elements of `values` are present in `x`.
...
validate whether a given set of `values` in a vector x matches a
...
These functions check if `x` is an S3, S4 or R6 object of the specified
class.
...
| `chk_r6_class(x, class)` | `inherits(x, "R6") && inherits(x, class)` |

[`chk_is()`](https://poissonconsulting.github.io/chk/reference/chk_is.md)
checks if x inherits from a specified class, regardless of whether it is
an S3 or S4 object.
...
specified by `regexp` matches all the non-missing values in the vector
`x`. If `regexp` it is not specified by the user, `chk_match` checks
whether all non-missing values in `x` contain at least one character
(regexp = “.+”)
...
For a data frame or matrix, the length corresponds to the number of
elements (not rows or columns), while for a vector or list, it
...
The function `chk_length` checks whether the length of `x` is within a
specified range. It ensures that the length is at least equal to
...
function is designed to validate whether the number of rows in the
resulting data frame from merging two data frames (`x` and `y`) is equal
to the number of rows in the first data frame (`x`). This is useful when
...
The `check_` functions combine several `chk_` functions internally. Read
...
Wickham, H. (2019). Advanced R, Second Edition (2nd ed.). Chapman and
Hall/CRC.
