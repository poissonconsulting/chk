...
times <- summary(microbenchmark(
  ...
  unit = "ns", times = 10000L
))[c("expr", "median")]
print(times)
```

In summary, avoiding `chk_flag()` saves about `r signif(diff(times$median[c(4,3)]),2)` ns while conditional checking using a `chk` argument costs about `r signif(diff(times$median[c(1,3)]),2)` ns.
