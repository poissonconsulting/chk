...
Do not confuse `chk_data()` with `check_data()`. See 
...
There are other functions you can use to work with numbers.

Function | Code
:- | :---
`chk_whole_numberic` | `is.integer(x) || (is.double(x) && vld_true(all.equal(x[!is.na(x)],  trunc(x[!is.na(x)]))))`
`chk_whole_number` |  `vld_number(x) && (is.integer(x) || vld_true(all.equal(x, trunc(x))))`
`chk_count` | `vld_whole_number(x) && x >= 0`

If you want to consider both 3 and 3L as integers,
...
vld_whole_numeric(c(1.0, 2.5, 3.0)) # FALSE
```

The function `chk_whole_number` is similar to  `chk_whole_numeric`; the only difference is that it doesn't consider NAs as valid elements.

```{r}
# Integer vector
vld_whole_numeric(c(1L, 2L, 3L, NA)) # TRUE
vld_whole_number(c(1L, 2L, 3L, NA)) # FALSE
```

`chk_count()` ensures that values are non-negative whole numbers.
It can be used for validating counts.
...
### class Date or DateTime Checkers
...
`chk_named` function works with vectors, lists, data frames, and matrices that have named columns or rows.
...
### Quality Checkers
Check if the function input meet some user defined quality criteria (Miscellanous),
...
```
The `chk_unique()` function is designed to verify that there are no duplicates
...
whether the number of rows in the resulting data frame from merging two data frames (`x` and `y`) is equal to the number of rows in the first data frame (`x`). 
